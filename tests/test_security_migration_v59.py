...
def test_v59_creates_signed_audit_head_and_revokes_public_execute(monkeypatch) -> None:
    migration = importlib.import_module("db.alembic.versions.v59_audit_chain_state_and_public_execute")
    fake_op = _PostgresOp()
    ...
    assert "ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC" in joined

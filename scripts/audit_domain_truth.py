...
defectos identificados (ver docs/IMPROVEMENT_BACKLOG.md):
  ...
  (e) Adjudicaciones con fecha imposible — anteriores a 1990 (C4.4).
...
# proporción baje de la mitad, así que el umbral ES la mitad — y por eso no es
# un `Umbral` calibrado como los de arriba: no vigila un empeoramiento sobre un
# valor medido, comprueba una corrección que aún no ha ocurrido.
...
def render(datos: dict[str, Any]) -> None:
    ...
    print("\n── (f) Discriminación del clasificador SAP (ml_proba) ──")

...
async def delete_my_session(
    ...
    def _trabajo() -> bool:
        """Revocar y auditar, en un solo salto al threadpool.

        La auditoría va aquí dentro y no después del `await`: `log_event`
        escribe en BD, y hacerlo desde el handler la pondría sobre el event
        loop (`tests/test_async_handlers_no_blocking_io.py`).
        """
        if not revoke_session_by_public_id(user_id, session_id):
            return False
        log_event(
...
async def create_my_key(
    ...
    def _crear() -> tuple[str, dict[str, Any] | None]:
        ...
        # La auditoría, dentro: `log_event` escribe en BD y desde el handler
        # correría sobre el event loop. Además así la clave y su registro de
        # creación salen del mismo salto — una clave acuñada sin rastro es
        # justo lo que este endpoint no debe poder producir.
        log_event(

...
function SourceRow({ source }: { source: SourceFreshness }) {
  ...
  return <tr className={cn("border-b border-border/60 last:border-0", source.is_degraded && "bg-destructive/5")}><td className="px-3 py-3 font-medium">{source.source}</td><td className="px-3 py-3"><Badge variant={state.variant}>{state.label}</Badge></td><td className="px-3 py-3 font-semibold tabular-nums">{lagLabel(source.lag_hours ?? null)}</td><td className="px-3 py-3 tabular-nums">{source.detected_within_24h_pct == null ? "—" : formatPercent(source.detected_within_24h_pct)}<span className="ml-1 text-xs text-muted-foreground">({source.sample_size})</span></td><td className="px-3 py-3 text-xs text-muted-foreground">{formatDate(source.last_success_at ?? null)}</td><td className="px-3 py-3 text-xs text-muted-foreground"><span title={source.warning ?? undefined}>{source.warning ?? `${formatNumber(source.parsed)} procesadas · ${formatNumber(source.errors)} errores`}</span></td></tr>;
}

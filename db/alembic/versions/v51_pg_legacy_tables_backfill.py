...
def upgrade() -> None:
    ...
    # Guardas de existencia: migraciones anteriores en la cadena (v26, v28,
    # v31) pudieron haber creado ya alguna de estas tablas -- especialmente
    # tras corregirlas para no envenenar la transacción en Postgres. Sin la
    # guarda, op.create_table fallaría con "ya existe".
    ...
    if "api_key_tiers" not in existing:
        op.create_table(
            "api_key_tiers",
    ...
    if "mat_clusters" not in existing:
        op.create_table(
            "mat_clusters",
    ...
    if "mat_top_empresas_ccaa" not in existing:
        op.create_table(
    ...
    if "notification_reads" not in existing:
        op.create_table(
            "notification_reads",
            sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
            sa.Column("user_key", sa.Text, nullable=False),
    ...
    if "pending_digests" not in existing:
        op.create_table(
            "pending_digests",
            sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
            sa.Column("user_key", sa.Text, nullable=False),
        ...
        op.create_index(
            "idx_pending_digests_recipient",
            "pending_digests",
            ["recipient_email", "sent", "frequency"],
        )
    ...
    if "watchlist_cpv" not in existing:
        op.create_table(
            "watchlist_cpv",
            sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
            sa.Column("user_key", sa.Text, nullable=False),
    ...
    if "extraction_runs" not in existing:
        op.create_table(
            "extraction_runs",
    ...
    if "failed_extractions" not in existing:
        op.create_table(
            "failed_extractions",
            sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
    ...
    if "csp_violations" not in existing:
        op.create_table(
            "csp_violations",
            sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),

...
4. **Auto-marking de tests**: `tests/conftest.py` infiere `unit`, `integration`, `e2e`, `property` y `load` por nombre. No introduzcas markers manuales de categoría: renombrá el test. Las excepciones históricas están congeladas por `scripts/check_agent_docs.py`; `slow` sí puede marcarse explícitamente.

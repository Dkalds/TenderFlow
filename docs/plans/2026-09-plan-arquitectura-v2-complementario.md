...
**Estado: EJECUTADO el 2026-09-08**, con tres cabos sueltos anotados abajo.

Los nueve streams se implementaron en
`claude/plan-arquitectura-complementario-b5888b` sobre `master` = `17169ce` y se
fusionaron sobre el árbol del plan v2. Lo que la fusión tuvo que resolver —la
cadena de migraciones, una ruta duplicada, un guard que v2 había retirado— está
en el mensaje del commit de fusión, no aquí.

**Lo que NO quedó cerrado**, y por qué:
...
`v112_organization_capabilities`: es un fichero generado y regenerarlo exige un
Postgres migrado. Tiene ficha propia en el backlog.

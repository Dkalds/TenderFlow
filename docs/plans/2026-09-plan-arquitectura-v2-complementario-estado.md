...
| C3.2 Agregados sin pandas | **Parcial + ratchet.** 9 escaneos sin cota encontrados, 1 corregido, 8 en allowlist decreciente con su motivo. **2026-09-18: 8 → 7.** `adjudicaciones_para_forecast` sale: leía por `licitacion_id = ANY(%s)` y el detector no reconocía esa forma como lectura por identidad (sí `IN (…)`); su cota es la lista de ids de `retendering_universe`, que sigue declarada. Las siete que quedan no admiten el paso a SQL sin cambiar comportamiento: el scoring (Radar, pipeline, drill-down de órgano), el parseo de miembros de UTE y la fecha de fin del forecast son Python sobre cada fila, y el p50 de `predicciones_baja` se cachea entero a propósito. `load_por_organo` es la candidata siguiente —top de adjudicatarios, mediana del lead-time y lookup por ids caben en SQL—, pero la mediana resta fechas TEXT con y sin zona, y su paridad con `pd.to_datetime(utc=True)` solo la fija un test contra Postgres, que esta rama no pudo correr. Su camino es la cota con `truncado` declarado, que exige la suite de integración contra Postgres para validarse. |
...
| Ítem | Estado |
|---|---|
| C6.1 Tareas de la oportunidad | **Hecho** (v122). `next_action` no se retira: se **deriva** de la tarea abierta más urgente, con un camino de escritura propio que no sube `version` ni escribe en el ledger — si lo hiciera, cerrar una tarea daría conflicto de concurrencia a quien editara la oportunidad en otra pestaña. **2026-09-18:** el evento `pursuit.task_due` entra al catálogo; lo escribe `services/pursuit_tasks.emitir_tareas_que_vencen` al principio de `event_dispatch.run()`, una vez por `(tarea, fecha)`, para el responsable de la tarea. El despachador respeta el interruptor `pursuit.task_due` de Ajustes (`notification_preferences`) en `in_app` y `email`. |
| C6.2 Menciones en comentarios | **Hecho** (v124). Se guardan los `user_id`, no el nombre resuelto. Un nombre ambiguo no resuelve a nadie: notificar a la persona equivocada es peor que un fallo visible. **2026-09-18:** la entrega va por el outbox. `add_comment` escribe `pursuit.mentioned` con los ids resueltos (sin el autor) y el despachador lo reparte según el interruptor `pursuit.mention` de Ajustes; `menciones_de_usuario` queda como la consulta del destinatario. **Salvedad que no es de C6:** `scheduler/jobs/event_dispatch.run()` no lo invoca hoy ningún plano (ni `pipeline_runs.py` ni el registro de jobs), así que ningún evento del outbox se entrega hasta que se cablee. |

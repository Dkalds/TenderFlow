...
**76 RFC**, de los cuales **20** siguen abiertos. `approved` 2 · `draft` 2 · `implemented` 50 · `obsolete` 4 · `partially-implemented` 15 · `review` 1 · `superseded` 2.
...
| [Retirada de los tres endpoints de export asíncrono y de su almacén en memoria](2026-09-03-rfc-retirada-exports-asincronos.md) | `implemented` | 2026-09-03 | `api/routes/exports.py` ya no declara `POST /exports` ni `GET` o `DELETE /exports/{job_id}`, y `tests/test_export_openapi.py` fija que `/api/v1/exports` y `/api/v1/exports/{job_id}` no están en el esquema. El sustituto, `GET /exports/download?format=pdf`, anuncia `application/pdf` en su 200. Retirada entregada en #265 (`88d7c7c0`, 2026-09-04); deprecación previa en `9207bde9` (2026-07-28). |
